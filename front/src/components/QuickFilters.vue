<template>
    <aside v-if="groups.length" class="panel" :class="{ collapsed }" :aria-label="ariaLabel">
...
<script>
import { filterFacetValues, formatCount, isLogFacetActive } from '@/utils/logQuickFilters';
...
export default {
    props: {
        ...
        localSearchKeys: { type: Array, default: () => [] },
    ...
    computed: {
        ...
        localSearch() {
            return new Set(this.localSearchKeys || []);
        },
        visibleGroups() {
    ...
    methods: {
        formatCount,
        hasLocalSearch(key) {
            return this.localSearch.has(key);
        },
